...
#     for i in range(1,len(z.T)):  # Here we plots the bands
#         subplot.plot(z.T[0], z.T[i]*freq2mev,
#                      color=color_bnd, linestyle=line_bnd) 
    for i in range(1,len(z.T)):  # Here we plots the bands
        subplot.plot(z.T[0], z.T[i]*freq2mev,
                     c=color_bnd, linestyle=line_bnd)      
    if 'legend' in kwargs:
    ...
    subplot.set_ylabel('Energy (1/cm)')
    if 'legend' in kwargs:
        subplot.legend()
...
bndplot(datafile, fermi, symmetryfile, ax, shift_fermi=fermi,
        color='black', linestyle='solid', name_k_points=['G', 'X', 'K', 'G','L'])
